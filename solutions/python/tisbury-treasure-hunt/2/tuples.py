...
def compare_records(azara_record, rui_record):
    ...
    """
    converted_coordinates = convert_coordinate(azara_record[1])
    return converted_coordinates == rui_record[1]
...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        new_record = (record[0],) + (record[2:])
        report_items.append(new_record)

    report = "\n".join(str(item) for item in report_items) + "\n"
    return report
