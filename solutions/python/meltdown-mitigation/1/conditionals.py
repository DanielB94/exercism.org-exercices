...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percenteage_value = (generated_power/theoretical_max_power)*100
    if percenteage_value >= 80:
        return 'green'
    elif percenteage_value < 80 and percenteage_value >= 60:
        return "orange"
    elif percenteage_value < 60 and percenteage_value >= 30:
        return "red"
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    ninety = threshold * 0.90
    lower = threshold * 0.10
    upper = threshold * 1.10
    if power < ninety:
        return 'LOW'
    elif  lower <= power <= upper:
        return "NORMAL"
    else:
        return "DANGER"
